...
test("should render NumeralDate with data-element prop", async ({
  ...
  await expect(getDataElementByValue(page, CHARACTERS.STANDARD)).toBeVisible();
});
...
] as [number, string, string][]).forEach(([inputIndex, tooltipText, field]) => {
...
test('should render NumeralDate with `["dd", "mm", "yyyy"]` dateFormat prop', async ({
  ...
  await expect(numeralDateInput(page, 0)).toHaveAttribute("placeholder", "dd");
  await expect(numeralDateInput(page, 1)).toHaveAttribute("placeholder", "mm");
  await expect(numeralDateInput(page, 2)).toHaveAttribute(
    "placeholder",
    "yyyy"
  );
});
...
test('should render NumeralDate with `["mm", "dd", "yyyy"]` dateFormat prop', async ({
  ...
  await expect(numeralDateInput(page, 0)).toHaveAttribute("placeholder", "mm");
  await expect(numeralDateInput(page, 1)).toHaveAttribute("placeholder", "dd");
  await expect(numeralDateInput(page, 2)).toHaveAttribute(
    "placeholder",
    "yyyy"
  );
...
test('should render NumeralDate with `["mm", "yyyy"]` dateFormat prop', async ({
  ...
  await expect(numeralDateInput(page, 0)).toHaveAttribute("placeholder", "mm");
  await expect(numeralDateInput(page, 1)).toHaveAttribute(
    "placeholder",
    "yyyy"
  );
  await expect(numeralDateInput(page, 2)).not.toBeVisible();
...
test("should have the expected border radius styling when no search button enabled", async ({
  ...
  await mount(<NumeralDateComponent />);
  await expect(numeralDateInput(page, 0).locator("..")).toHaveCSS(
    "border-radius",
    "4px 0px 0px 4px"
  );
  await expect(numeralDateInput(page, 2).locator("..")).toHaveCSS(
    "border-radius",
    "0px 4px 4px 0px"
  );
});

test.describe("check events for NumeralDate component", () => {
...
test.describe("check accessibility for NumeralDate component", () => {
  ...
  // TODO: remove skipped disabled test parameter when FE-5732 is done
