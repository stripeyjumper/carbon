...
test.describe("Prop checks for Portrait component", () => {
  ...
  testData.forEach((stringVals) => {
    ...
    test("should render the correct element when 'showProgressIndicator' is true", async ({mount, page}) => {
  ...
  ([
    ...
    test(`should render the correct label when the 'totalSteps' prop is passed as ${totalSteps} and 'currentStep' is ${currentStep}`, async ({
      ...
      await mount(<StepFlowComponent totalSteps={totalSteps} currentStep={currentStep} />);

      if(totalSteps >= currentStep){
        await expect(stepFlowLabel(page)).toHaveText(`${currentStep} of ${totalSteps}`);
      } else {
        await expect(stepFlowLabel(page)).toHaveText(`${totalSteps} of ${totalSteps}`);
      }
    });
});
...
([1, 2, 3, 4, 5, 6, 7, 8] as Steps[]).forEach((totalSteps) => {
    test(`should render the correct amount of progress indicators when the 'totalSteps' prop is passed as ${totalSteps}`, async ({
      ...
      await mount(<StepFlowComponent totalSteps={totalSteps} showProgressIndicator/>);

     expect(await stepFlowProgressIndicator(page).count()).toEqual(totalSteps);
    });
});
    
test("should render the correct element when 'showCloseIcon' is true", async ({mount, page}) => {
    await mount(<StepFlowComponent showCloseIcon />);

    await expect(stepFlowDismissIcon(page)).toBeVisible();
});

test("background colour token should be correct when 'showCloseIcon' is true and the indicator is in progress", async ({mount, page}) => {
    await mount(<StepFlowComponent currentStep={1} totalSteps={1} showProgressIndicator />);
...
test("background colour token should be correct when 'showCloseIcon' is true and the indicator is completed", async ({mount, page}) => {
    await mount(<StepFlowComponent currentStep={2} totalSteps={3} showProgressIndicator />);
...
test("background colour should be correct when 'showCloseIcon' is true and the indicator is not completed", async ({mount, page}) => {
    await mount(<StepFlowComponent currentStep={2} totalSteps={3} showProgressIndicator />);
...
test.describe("Accessibility tests for StepFlow component", () => {
  ...
  test("should pass accessibility checks when 'showProgressIndicator' is true and there are completed, not completed and in-progress indicator steps", async ({
    ...
    await mount(<StepFlowComponent showProgressIndicator currentStep={2} totalSteps={3} />)
